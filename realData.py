...
def fetch_temp_seq():
    filename = "data/temp_file_processed_mod_n" + str(param.n_items) + "_m" + str(param.size_dataset)
...
def pre_process_temp_seq():
    """
    Make sure we have the correct # of hosts in the sequence
    """
        ...
        for sample in dataset[temp_p]["sequences"]:
            ...
            if len(distinct_items) == param.n_items:  # nothing to do
                ...
                for i in range(0, len(dataset[temp_p]["sequences"][sample]) - 1):  # stop at second-last request
                    ...
                    if curr_item != dataset[temp_p]["sequences"][sample][
                        i + 1] and curr_item not in appearing_items:  # add to candidates once we switch to another item
                ...
                while missing_items:
                    ...
                    if single:
                        ...
                        if (tentative, tentative2) not in distinct_items and \
                                (tentative,
                                 tentative2) not in random_sample:  # get a random sample of items not in sequence
                ...
                logger.write("Test, sample" + str(sample) + " now has " + str(len(test)) + " items and " +
                             str(len(new_dataset[temp_p]["sequences"][sample])) + " requests, replaced " + str(
                    counter) + " items  \n")
...
def create_original():
    """
    Create first set of requests for all temporal params provided below
    """
    incr_p_set = [0, 0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4,
                  0.45, 0.5, 0.55, 0.6, 0.65, 0.7, 0.75, 0.8, 0.85, 0.9, 0.95]

    # params
    num_simulations = 1  # more datasets of the same type can be created
    res_dict = {}
...
def mod_temp_seq():
    """
    Add actual metrics to json, making it easier to distinguish the data
    """
    ...
    with open(filename + "_mod_n" + str(param.n_items) + "_m" + str(param.size_dataset) + ".json", "x") as f:
